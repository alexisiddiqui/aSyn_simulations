...
def resolve_posres_itp(
    repo_root: Path, condition_name: str, topo_cfg: dict, work_dir: Path
) -> Path:
...
def fix_topology_includes(topology_file: Path) -> None:
    """Fix absolute paths in topology file includes to use relative paths.

    Converts hardcoded absolute paths (e.g., /home/alexi/...) to relative
    paths that work across different systems.
    """
    content = topology_file.read_text()

    # Replace absolute path includes with relative paths
    # Handle /home/alexi/... paths (macOS)
    content = content.replace(
        '#include "/home/alexi/Documents/aSyn_simulations/topology_output/protein/ph_7.4/posre.itp"',
        '#include "posre.itp"',
    )
    # Comment out missing posreca.itp
    content = content.replace(
        '#include "/home/alexi/Documents/aSyn_simulations/topology_output/protein/ph_7.4/posreca.itp"',
        '; #include "posreca.itp"',
    )

    topology_file.write_text(content)
...
def process_condition(
    ...
    if not system_gro.exists() or not system_top.exists():
        raise FileNotFoundError(
            f"System files not found for {condition_name}. Run build_system.py first."
        )
    ...
    if not work_system_gro.exists():
        ...
        print("  Copied system.gro")

    if not work_system_top.exists():
        work_system_top.write_text(system_top.read_text())
        print("  Copied system.top")

    # Fix hardcoded absolute paths in topology file
    fix_topology_includes(work_system_top)
    ...
    if single_stage:
        ...
        if through_stage not in stage_names:
            raise ValueError(f"Unknown stage: {through_stage}. Available: {stage_names}")
...
def main():
    parser = argparse.ArgumentParser(description="Run GROMACS simulation pipeline for aSyn MD")
    parser.add_argument(
        "--config",
        type=Path,
    ...
    if args.condition:
        if args.condition not in all_conditions:
            raise ValueError(f"Unknown condition: {args.condition}. Available: {all_conditions}")
